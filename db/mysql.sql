...
CREATE TABLE `uc_user` (
  ...
  `email` varchar (50) NOT NULL COMMENT '绑定的邮箱',
  `site`varchar (10) COMMENT '注册来自的站点',
...
CREATE TABLE `uc_user_account` (
  ...
  `acct_type` varchar(6) NOT NULL COMMENT '账户类型对应不同币种',
  `acct_status` varchar(2) NOT NULL COMMENT '账户状态:正常(A)/冻结(F)/注销(C)',
  `available_amt` decimal(16,4) COMMENT '可用金额',
  `wallet_addr` varchar (50) COMMENT '绑定的钱包地址',
  `create_time` datetime COMMENT '创建时间',
  PRIMARY KEY (`acct_id`)
...
-- 用户账户明细表
CREATE TABLE `uc_account_logs` (
  `log_id` bigint NOT NULL AUTO_INCREMENT,
  `user_id` bigint NOT NULL COMMENT '用户id',
  `acct_id` bigint NOT NULL COMMENT '账户id',
  `acct_type` varchar(6) NOT NULL COMMENT '账户类型对应不同币种',
  `oper_type` varchar(2) COMMENT '操作类型:充值(R)/转账(T)/付款(P)等',
  `loan_type` int(1) NOT NULL COMMENT '记账类型:消纳(0)/发生(1)',
  `amt`decimal(16,4) COMMENT '发生金额',
  `rapid_amt` decimal(16,4) COMMENT '快速记账:+/-金额',
  `pre_oper_amt` decimal(16,4) COMMENT '操作之前余额',
  `order_id` bigint COMMENT '关联订单，充值订单等的订单id',
  `oper_user_id` bigint NOT NULL COMMENT '操作人id',
  `create_time` datetime COMMENT '操作时间',
  PRIMARY KEY (`log_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='用户账户明细';
...
INSERT INTO `uc_user` (`username`, `mobile`, `password`,`email`,`site`,`referrer_id`,`create_time`) VALUES ('mark', '13612345678', '8c6976e5b5410415bde908bd4dee15dfb167a9c873fc4bb8a81f6f2ab448a918','13612345678@qq.com','hstr','0', '2017-03-23 22:37:41');

-- 积分账户
INSERT INTO `uc_user_account` (`user_id`, `acct_type`,`acct_status`,`available_amt`,`wallet_addr`,`create_time`) VALUES(1,'point','A','50000','0x3592B74C5A71a596a883bC6c17dEfeeD947b6E0b','2017-03-23 22:37:41');
-- 积分充值明细
INSERT INTO `uc_account_logs` (`user_id`, `acct_id`,`acct_type`,`oper_type`,`loan_type`,`amt`,`rapid_amt`,`pre_oper_amt`,`oper_user_id`,`create_time`) VALUES(1,1,'point','R','1',5000,+5000,0,1,'2017-03-23 22:37:41');
